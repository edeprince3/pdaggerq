...
std::vector<pq_operator_terms> pq_helper::get_bernoulli_operator_terms_1(double factor, const std::vector<std::string> &targets,const std::vector<std::string> &ops) {
    ...
    for (size_t p = 0; p < targets_partitions.size(); p++){
        ...
        for (int i = 0; i < dim; i++) {
            std::vector<pq_operator_terms> tmp = get_commutator_terms(bernoulli_factors[p] * factor, b_targets, {b_ops[i]});
            bernoulli_terms.insert(std::end(bernoulli_terms), std::begin(tmp), std::end(tmp));
        }
    }
...
std::vector<pq_operator_terms> pq_helper::get_bernoulli_operator_terms_2(double factor, const std::vector<std::string> &targets,const std::vector<std::string> &ops) {
    ...
    for (size_t p = 0; p < partitions_lists.size(); p++){
        ...
        for (int i = 0; i < dim; i++) {
            for (int j = 0; j < dim; j++) {
                std::vector<pq_operator_terms> tmp = get_double_commutator_terms(bernoulli_factors[p] * factor, b_targets, {b_ops1[i]}, {b_ops2[j]});
                bernoulli_terms.insert(std::end(bernoulli_terms), std::begin(tmp), std::end(tmp));
            }
        }
    }
    ...
    for (size_t p = 0; p < partitions_lists.size(); p++){
        ...
        for (int i = 0; i < dim; i++) {
            for (int j = 0; j < dim; j++) {
                for (int k = 0; k < dim; k++) {
                    for (int l = 0; l < dim; l++) {
                        std::vector<pq_operator_terms> tmp = get_quadruple_commutator_terms(bernoulli_factors[p] * factor, b_targets, {b_ops1[i]}, {b_ops2[j]}, {b_ops3[k]}, {b_ops4[l]});
                        bernoulli_terms.insert(std::end(bernoulli_terms), std::begin(tmp), std::end(tmp));
                    }
                }
            }
        ...
        for (int i = 0; i < dim; i++) {
            for (int j = 0; j < dim; j++) {
                for (int k = 0; k < dim; k++) {
                    for (int l = 0; l < dim; l++) {
                        for (int m = 0; m < dim; m++) {
                            std::vector<pq_operator_terms> tmp = get_quintuple_commutator_terms(bernoulli_factors[p] * factor, b_targets, {b_ops1[i]}, {b_ops2[j]}, {b_ops3[k]}, {b_ops4[l]}, {b_ops5[m]});
                            bernoulli_terms.insert(std::end(bernoulli_terms), std::begin(tmp), std::end(tmp));
                        }
                    }
                }
            ...
            for (int j = 0; j < dim; j++) {
                for (int k = 0; k < dim; k++) {
                    for (int l = 0; l < dim; l++) {
                        for (int m = 0; m < dim; m++) {
                            for (int n = 0; n < dim; n++) {
                                std::vector<pq_operator_terms> tmp = get_hextuple_commutator_terms(bernoulli_factors[p] * factor, b_targets, {b_ops1[i]}, {b_ops2[j]}, {b_ops3[k]}, {b_ops4[l]}, {b_ops5[m]}, {b_ops6[n]});
                                bernoulli_terms.insert(std::end(bernoulli_terms), std::begin(tmp), std::end(tmp));
                            }
                        }
                    }
